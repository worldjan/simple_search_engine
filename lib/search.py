...
from gio._gio import socket_connection_factory_register_type
import math
from scipy.special._cephes import wofz
...
class TfidfRank(object):
    ...
    def build_rank(self, objects):
        ...
        score_json = {}
        data_list = self.tf_idf_matrix.data.tolist()
        data_index = 0
        for doc_idx in range(self.tf_idf_matrix.shape[0]):
            term_indices = self.tf_idf_matrix.indices[self.tf_idf_matrix.indptr[doc_idx]:self.tf_idf_matrix.indptr[doc_idx + 1]]
            for term_index in term_indices:
                if term_index in score_json:
                    score_json[term_index][doc_idx] = data_list[data_index]
                else:
                    score_json[term_index] = {doc_idx: data_list[data_index]}

                data_index += 1

        with open('./origin_score.txt', mode='w') as score_output_file:
            pprint.pprint(score_json, score_output_file)

        print self.tf_idf_matrix

    def build_rank_ys(self, objects):
        self.__build_vocabulary(objects)
        ...
        # compute idf
        logging.info("[Ranking] Starting tf computation...")
        for doc_idx, indexable in enumerate(objects):
            for word in indexable.words_generator(self.stop_words):
                word_index_in_vocabulary = self.vocabulary[word]
                doc_word_count = indexable.count_for_word(word)
                if word_index_in_vocabulary in self.tf_matrix:
                    self.tf_matrix[word_index_in_vocabulary][doc_idx] = doc_word_count
                else:
                    self.tf_matrix[word_index_in_vocabulary] = {doc_idx: doc_word_count}
        ...
        n_docs_smooth = n_docs + self.smoothing
        norm = {}
        for word_idx in self.tf_matrix.keys():
            df = len(self.tf_matrix[word_idx]) + self.smoothing
            idf = math.log(float(n_docs_smooth) / df) + 1
            for doc_idx in self.tf_matrix[word_idx].keys():
                tfidf_score = self.tf_matrix[word_idx][doc_idx] * idf
                if word_idx in self.tf_idf_score_matrix:
                    self.tf_idf_score_matrix[word_idx][doc_idx] = tfidf_score
                else:
                    self.tf_idf_score_matrix[word_idx] = {doc_idx: tfidf_score}

                if doc_idx in norm:
                    norm[doc_idx] += tfidf_score ** 2
                else:
                    norm[doc_idx] = tfidf_score ** 2

        # compute td-idf normalization
        for index in norm.keys():
            norm[index] = 1.0 / math.sqrt(norm[index])

        for word_idx in self.tf_idf_score_matrix.keys():
            for index in self.tf_idf_score_matrix[word_idx].keys():
                self.tf_idf_score_matrix[word_idx][index] *= norm[index]

        with open('./ys_score.txt', mode='w') as score_output_file:
            pprint.pprint(self.tf_idf_score_matrix, score_output_file)
    ...
    def compute_rank_ys(self, doc_index, terms):
        score = 0
        for term in terms:
            term_index = self.vocabulary[term]
            score += self.tf_idf_score_matrix[term_index][doc_index]
        return score
...
class Index(object):
    ...
    def search_terms(self, query_terms):
        ...
        """
        docs_indices = []
        for query_term_index, query_term in enumerate(query_terms):

            # keep only docs that contains all terms
            if query_term not in self.term_index:
                docs_indices = []
                break

            # compute intersection between results
            # there is room for improvements in this part of the code
            docs_with_term = self.term_index[query_term]
            if query_term_index == 0:
...
class SearchEngine(object):
    ...
    def start(self):
        ...
        if not MODE_YS:
            ...
            self.rank.build_rank_ys(self.objects)
    ...
    def search(self, query, n_results=10):
        ...
        for doc_index in docs_indices:
            ...
            if not MODE_YS:
                ...
                doc_score = self.rank.compute_rank_ys(doc_index, terms)
            result = IndexableResult(doc_score, indexable)
            search_results.append(result)

        search_results.sort(key=lambda x: x.score, reverse=True)
        return search_results[:n_results], len(search_results)
